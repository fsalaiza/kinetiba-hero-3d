...
.kba-nav-dot { width: 8px; height: 8px; border-radius: 50%; border: 1.5px solid rgba(230,230,220,0.3); background: transparent; cursor: pointer; transition: all 0.2s ease; padding: 0; }
...
function getActiveSection(scrollProgress) {
  ...
  if (scrollProgress < 0.34) return 0;  // BI
  if (scrollProgress < 0.50) return 1;  // WA
  if (scrollProgress < 0.84) return 2;  // ERP
  if (scrollProgress < 0.86) return 3;  // AGT
  return 4;                              // CTA
}
...
export default function SectionNav({ scrollProgress }) {
  ...
  return (
    <>
      ...
      {/* Dots — right side */}
      <div className="kba-nav-dots" style={{ opacity: active >= 0 ? 1 : 0, transition: "opacity 0.3s ease" }}>
        {SECTIONS.map((_, i) => (
          <button
            key={i}
            className={`kba-nav-dot${active === i ? " active" : ""}`}
            aria-label={`Ir a sección ${SECTIONS[i]}`}
          />
        ))}
      </div>

      {/* Icons — bottom left */}
      <div className="kba-nav-icons" style={{ opacity: active >= 0 ? 1 : 0, transition: "opacity 0.3s ease" }}>
        {SECTIONS.map((label, i) => (
          <div key={i} className={`kba-nav-icon${active === i ? " active" : ""}`}>
            <span>{label}</span>
          </div>
        ))}
      </div>
