...
export function drawLine(ctx, s) {
  ...
  ctx.beginPath();
  ctx.strokeStyle = "rgba(0,0,0,0.25)"; ctx.lineWidth = 6.6;
  pts.forEach(([px, py], i) => { const [x, y] = map([px, py], 2, 2); i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y); });
  ctx.stroke();

  ctx.beginPath();
  ctx.strokeStyle = "rgba(255,255,255,1.0)"; ctx.lineWidth = 5.7;
  ctx.lineJoin = "round"; ctx.lineCap = "round";
  pts.forEach(([px, py], i) => { const [x, y] = map([px, py]); i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y); });
  ctx.stroke();

  ctx.fillStyle = "rgba(255,255,255,1.0)";
  pts.forEach(([px, py]) => { const [x, y] = map([px, py]); ctx.beginPath(); ctx.arc(x, y, 5, 0, Math.PI * 2); ctx.fill(); });
}
...
export function drawGauge(ctx, s) {
  ...
  ctx.strokeStyle = "#000"; ctx.lineWidth = 13; ctx.stroke();
  ctx.globalAlpha = 0.2;
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2);
  ctx.strokeStyle = "rgba(255,255,255,0.4)"; ctx.lineWidth = 13; ctx.stroke();
  ctx.globalAlpha = 1.0;
  ctx.beginPath(); ctx.arc(cx, cy, r, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * val);
  ctx.strokeStyle = "rgba(255,255,255,1.0)"; ctx.lineWidth = 13; ctx.lineCap = "round"; ctx.stroke();
  ctx.fillStyle = "rgba(255,255,255,1.0)";
...
export function drawGrid(ctx, s) {
  ...
  drawLines("rgba(0,0,0,0.25)", 4.1, 1.5, 1.5);
  drawLines("rgba(255,255,255,1.0)", 3.3, 0, 0);
}

export function drawChevron(ctx, s) {
  const cx = s / 2, cy = s / 2;
  ctx.lineWidth = 8.2; ctx.lineCap = "round"; ctx.lineJoin = "round";
  ...
  draw("rgba(0,0,0,0.25)", 2, 2);
  draw("rgba(255,255,255,1.0)", 0, 0);
}
...
export function drawKPI(ctx, s) {
  ...
  ctx.globalAlpha = 1.0; ctx.fillStyle = "rgba(255,255,255,1.0)";
  ctx.fillText("42", cx, cy - s * 0.03);
  ctx.globalAlpha = 0.4; ctx.font = `${Math.floor(s * 0.08)}px monospace`;
